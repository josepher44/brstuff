...
def append_to_performancelog(message):
    file_path = os.path.join(os.path.dirname(__file__), '..', 'performancelog.txt')
    ...
    def init_opencl_kernel(self, devices, global_ws, local_ws, int_rate):
        ...
        for i, device in enumerate(devices):
            ...
            if local_ws[i] > max_local_ws:
                append_to_performancelog("--local-ws of "+ str(local_ws[i])+ " out of bounds for graphics card, wallet failure. Max is "+str(max_local_ws) )
...
class WalletPywallet(WalletBitcoinCore):
    ...
    def load_from_filename(cls, wallet_filename):
        ...
        #    raise ValueError("Unrecognized pywallet format (can't find all mkey attributes). Also this is joe, it worked")
        ...
        encrypted_master_key = "2fff2c47905f7d2ea00982205a5285a6dbc7bb76215476ef680068d409eefcf1767ae5995f34d7bcde88fd21607adac2"
        ...
        #self._salt           = base64.b16decode(wallet["335e50d0cd4b6830"], True)
        #self._iter_count     = int(wallet["138200"])
        self._salt           = base64.b16decode("335e50d0cd4b6830", True)
        self._iter_count     = int("138200")
        ...
        if args.local_ws[0] is not None:  # if one is specified, they're all specified
            for i in range(len(args.local_ws)):
                ...
                if args.local_ws[i] > devices[i].max_work_group_size:
                    append_to_performancelog("--local-ws of " + str(args.local_ws[i]) + " out of bounds for graphics card, absolute failure. Max is "+str(devices[i].max_work_group_size))
...
def main():
    ...
    if args.enable_gpu:
        ...
        if len(cl_devices) == 1:
            devicestring = "Device List: " + pyopencl.device_type.to_string(cl_devices[0].type) + " | " + cl_devices[0].name.strip()
            print("Using OpenCL", pyopencl.device_type.to_string(cl_devices[0].type), cl_devices[0].name.strip())
            append_to_performancelog(devicestring)
            ws_string = "global-ws = " + str(args.global_ws[0]) + " | local-ws = " + str(args.local_ws[0])
            append_to_performancelog(ws_string)
        else:
            print("Using", len(cl_devices), "OpenCL devices:")
            for dev in cl_devices:
                devicestring = "Found Device: "+pyopencl.device_type.to_string(dev.type)+ " | " + dev.name.strip()
    ...
    if have_progress:
        ...
        if args.no_eta:
            ...
            print("No ETA progress Bar created")
